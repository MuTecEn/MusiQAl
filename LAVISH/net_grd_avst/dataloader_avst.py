...
class AVQA_dataset(Dataset):
	...
	def __init__(self, label, audio_dir, video_dir, video_res14x14_dir, transform=None, mode_flag='train'):


		samples = json.load(open('./AVQA/data/json/avqa-train.json', 'r'))
	...
	def __getitem__(self, idx):
		...
		total_num_frames = len(glob.glob(os.path.join(self.video_dir,name,'*.jpg')))
		sample_indx = np.linspace(1, total_num_frames , num=10, dtype=int)
		total_img = []
		for tmp_idx in sample_indx:
			# tmp_img = torchvision.io.read_image(os.path.join(self.video_dir,name, str("{:04d}".format(tmp_idx))+ '.jpg'))/255
			tmp_img = torchvision.io.read_image(os.path.join(self.video_dir,name, str("{:08d}".format(tmp_idx))+ '.jpg'))/255
		...
		for i in range(total_img.shape[0]):
			...
			tmp_img = torchvision.io.read_image(os.path.join(self.video_dir,neg_video_name, str("{:08d}".format(sample_indx[neg_frame_flag]))+ '.jpg'))/255
		...
		for audio_sec in range(10):
			pth='./AVQA/data/audio'
			fbank, mix_lambda = self._wav2fbank(f'{pth}/{name}.wav', idx=audio_sec)
